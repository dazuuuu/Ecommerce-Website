...
<div class="flex items-center gap-2 mb-5 text-xs font-semibold">
  <a href="<?= url('/admin/orders') ?>" class="px-3 py-1.5 rounded-full <?= !$statusFilter ? 'bg-amber-400 text-black' : 'bg-white border border-neutral-300 text-neutral-600' ?>">All</a>
  <?php foreach ($statuses as $s): ?>
    <a href="<?= url('/admin/orders?status=' . $s) ?>" class="px-3 py-1.5 rounded-full <?= $statusFilter === $s ? 'bg-amber-400 text-black' : 'bg-white border border-neutral-300 text-neutral-600' ?>"><?= ucfirst($s) ?></a>
  <?php endforeach; ?>
</div>
    ...
    <tbody class="divide-y divide-neutral-100">
      ...
      <?php foreach ($orders as $o): ?>
        <tr class="hover:bg-neutral-50 cursor-pointer" onclick="location.href='<?= url('/admin/orders/' . urlencode($o['order_ref'])) ?>'">
          <td class="px-5 py-3 font-mono font-bold text-emerald-800"><?= e($o['order_ref']) ?></td>
