...
<div class="max-w-md mx-auto text-center">
  <span class="text-xs font-bold text-amber-700 uppercase tracking-widest block mb-1">Verify It's You</span>
  ...
  <div class="bg-white border border-neutral-200 rounded-xl shadow-sm p-6 text-left">
    ...
    <form method="post" action="<?= url('/account/verify') ?>" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="code" required autofocus maxlength="6" inputmode="numeric" pattern="[0-9]{6}" placeholder="000000" class="w-full mt-1 bg-white border border-neutral-300 rounded-lg p-3 text-center text-2xl font-mono tracking-[0.5em] focus:outline-none focus:border-emerald-800" />
      </div>
      <button type="submit" class="w-full bg-[#0a0a0a] hover:bg-[#0d2a18] text-amber-300 text-xs font-bold py-3 rounded-lg uppercase tracking-widest transition-colors cursor-pointer border border-amber-400/30">Verify &amp; Sign In</button>
    </form>

    <form method="post" action="<?= url('/account/verify') ?>" class="mt-3">
      <?= csrfField() ?>
      <input type="hidden" name="action" value="resend" />
      <button type="submit" class="w-full text-xs font-bold text-emerald-800 hover:underline py-2 cursor-pointer">Resend code</button>
    </form>
  </div>
