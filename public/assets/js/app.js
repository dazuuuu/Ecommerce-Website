...
(function () {
  ...
  function renderProductCardHTML(product, isWishlisted) {
    ...
    var colorsHTML = (product.colors || []).slice(0, 6).map(function (c) {
      return '<span title="' + esc(c.name || 'Color') + '" style="background-color:' + esc(c.hex || '#111') + '"></span>';
    }).join('');
      ...
      '<div class="product-card group" data-product-id="' + esc(product.id) + '">' +
        '<div>' +
          ...
          '<div class="product-card-body">' +
            ...
            '</div>' +
            (colorsHTML ? '<div class="product-color-swatches" aria-label="Available colors">' + colorsHTML + '</div>' : '') +
          '</div>' +
        '</div>' +
  ...
  var categoryHeadings = { all: 'All Products', new: 'New Arrivals' };
