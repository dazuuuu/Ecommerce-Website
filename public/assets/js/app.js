...
(function () {
  ...
  function initHeader() {
    ...
    window.addEventListener('scroll', function () {
      ...
      headerBar.classList.toggle('bg-[#092214]', scrolled);
      headerBar.classList.toggle('py-2.5', scrolled);
      headerBar.classList.toggle('sm:py-3', scrolled);
      headerBar.classList.toggle('border-[#1e462d]', scrolled);
      headerBar.classList.toggle('shadow-lg', scrolled);
      headerBar.classList.toggle('bg-[#0c2e1b]', !scrolled);
      headerBar.classList.toggle('py-3', !scrolled);
      headerBar.classList.toggle('sm:py-4', !scrolled);
      headerBar.classList.toggle('border-[#1b432a]', !scrolled);
    });
  ...
  function setActiveNav(category) {
    document.querySelectorAll('#desktop-nav-links [data-nav-id]').forEach(function (btn) {
      ...
      btn.classList.toggle('text-amber-100/90', !active);
    ...
    document.querySelectorAll('.nav-select-mobile[data-nav-id]').forEach(function (btn) {
      ...
      btn.classList.toggle('font-bold', active);
      btn.classList.toggle('text-amber-100', !active);
      btn.classList.toggle('hover:bg-[#184229]', !active);
    });
    document.querySelectorAll('.cat-pill[data-cat]').forEach(function (btn) {
      ...
      btn.classList.toggle('text-black', active);
      btn.classList.toggle('font-bold', active);
      btn.classList.toggle('text-amber-100/80', !active);
  ...
  function updateHeaderUI() {
    ...
    document.querySelectorAll('.currency-option, .currency-option-mobile, .currency-option-footer').forEach(function (btn) {
      ...
      if (btn.classList.contains('currency-option-footer')) {
        btn.classList.toggle('bg-amber-400', active);
        btn.classList.toggle('text-black', active);
        btn.classList.toggle('border-amber-400', active);
        btn.classList.toggle('bg-[#0d2a18]', !active);
        btn.classList.toggle('text-amber-200', !active);
        btn.classList.toggle('border-amber-500/20', !active);
      } else if (btn.classList.contains('currency-option-mobile')) {
        btn.classList.toggle('bg-amber-400', active);
        btn.classList.toggle('text-black', active);
        btn.classList.toggle('bg-[#0a0a0a]', !active);
        btn.classList.toggle('text-amber-300', !active);
        btn.classList.toggle('border-[#1b432a]', !active);
      } else {
        btn.classList.toggle('font-bold', active);
        btn.classList.toggle('text-amber-300', active);
        btn.classList.toggle('bg-[#123620]', active);
        btn.classList.toggle('text-amber-100/80', !active);
      }
    });
  }
  ...
  function heroCardContentHTML(item) {
    return (
      ...
      '<h3 class="font-serif-heading font-bold text-xs sm:text-base text-neutral-900 leading-tight truncate group-hover:text-emerald-800 transition-colors">' + esc(item.name) + '</h3>' +
      ...
      '<span class="text-[10px] sm:text-xs font-bold text-neutral-900 underline group-hover:text-emerald-800 flex items-center gap-1">Shop ' + icon('arrowRight', 'w-3 h-3 text-emerald-700') + '</span>' +
      '</div></div>'
    );
  }
  ...
  function setupHeroCard(cardNum, items, intervalMs) {
    ...
    function render() {
      ...
      dotsEl.innerHTML = items.map(function (_, idx) {
        return '<button data-hero-dot class="h-1.5 rounded-full transition-all cursor-pointer ' + (idx === index ? 'w-5 bg-emerald-800' : 'w-1.5 bg-neutral-300') + '" data-idx="' + idx + '"></button>';
      }).join('');
    ...
    if (state.currentCategory === 'new') {
      ...
      list = list.filter(function (p) {
        return p.category === state.currentCategory ||
          p.subCategory.toLowerCase().indexOf(state.currentCategory.toLowerCase()) !== -1;
      });
    }
  ...
  function renderProductCardHTML(product, isWishlisted) {
    ...
    var wishClasses = isWishlisted ? 'text-rose-600' : 'text-neutral-400 hover:text-neutral-700';
    var heartClasses = isWishlisted ? 'fill-rose-600 text-rose-600' : '';

    return (
      '<div class="product-card group relative bg-white border border-[#e2e8f0] rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between p-2.5 sm:p-3.5" data-product-id="' + esc(product.id) + '">' +
        '<div>' +
          '<div class="product-card-image-wrap relative w-full aspect-[4/3] bg-neutral-100 rounded-lg overflow-hidden cursor-crosshair group select-none" data-quickview-trigger data-product-id="' + esc(product.id) + '">' +
            ...
            '<div class="absolute top-2 left-2 flex flex-col gap-1 z-10 pointer-events-none">' +
              (product.isNew ? '<span class="bg-[#0f2e1b] text-amber-300 text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 rounded shadow-sm border border-amber-400/30">NEW</span>' : '') +
              (product.isBestSeller ? '<span class="bg-amber-600 text-white text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 rounded shadow-sm">BEST SELLER</span>' : '') +
            '</div>' +
            '<div class="absolute bottom-2 right-2 flex items-center gap-1.5 z-10">' +
              ...
              '<button data-quickview-trigger data-product-id="' + esc(product.id) + '" class="p-2 rounded-full bg-black/80 text-amber-300 hover:bg-black hover:scale-110 transition-all opacity-0 group-hover:opacity-100 cursor-pointer shadow-md" title="Quick View Details">' + icon('eye', 'w-4 h-4') + '</button>' +
            '</div>' +
          '</div>' +
          '<div class="pt-3 pb-2 flex flex-col">' +
            '<h3 data-quickview-trigger data-product-id="' + esc(product.id) + '" class="font-medium text-sm sm:text-base text-neutral-900 hover:text-[#0f2e1b] transition-colors cursor-pointer leading-snug line-clamp-2 min-h-[2.5rem]">' + esc(product.name) + '</h3>' +
            '<div class="py-1.5 flex items-center">' +
              '<button data-wishlist-toggle data-product-id="' + esc(product.id) + '" class="wishlist-btn p-1 -ml-1 transition-colors cursor-pointer rounded-full ' + wishClasses + '" aria-label="Wishlist">' +
                icon('heart', 'w-5 h-5 ' + heartClasses, isWishlisted ? 'currentColor' : 'none') +
              '</button>' +
            '</div>' +
            '<div class="flex items-baseline gap-2 mb-2">' +
              '<span class="text-base sm:text-xl font-bold text-[#8b1c1c] tracking-tight">' + esc(formattedPrice) + '</span>' +
              (formattedOriginal ? '<span class="text-xs text-neutral-400 line-through font-normal">' + esc(formattedOriginal) + '</span>' : '') +
            '</div>' +
          '</div>' +
        '</div>' +
        '<div class="flex flex-col gap-2 pt-1 mt-auto">' +
          '<button data-buy-now data-product-id="' + esc(product.id) + '" class="w-full bg-[#0a0a0a] hover:bg-[#0f2e1b] active:bg-[#143d23] text-white font-bold py-2.5 px-3 rounded-lg sm:rounded-xl flex items-center justify-center gap-2 text-xs sm:text-sm tracking-wider uppercase transition-all duration-200 cursor-pointer shadow-md hover:shadow-lg border border-[#0a0a0a]">' +
            icon('zap', 'w-4 h-4 text-amber-300 fill-amber-300') + '<span>BUY NOW</span></button>' +
          '<button data-quick-add data-product-id="' + esc(product.id) + '" class="w-full bg-white hover:bg-emerald-50/50 active:bg-neutral-100 text-[#0a0a0a] font-bold py-2.5 px-3 rounded-lg sm:rounded-xl flex items-center justify-center gap-2 text-xs sm:text-sm tracking-wider uppercase transition-all duration-200 cursor-pointer border-2 border-[#0a0a0a]">' +
            icon('shoppingCart', 'w-4 h-4 text-[#0a0a0a]') + '<span>ADD TO CART</span></button>' +
        '</div>' +
      '</div>'
    );
  }
  ...
  var categoryHeadings = { all: 'All Luxury Goods', new: 'New Arrivals' };
  ...
  function renderProductsSection() {
    ...
    if (list.length === 0) {
      ...
      grid.innerHTML =
        ...
        '<p class="font-serif-heading text-lg font-bold">No pieces found in this category.</p>' +
        '<button id="empty-view-all" class="bg-[#0a0a0a] text-amber-300 border border-amber-400/40 text-xs font-bold px-6 py-2.5 rounded-lg uppercase tracking-widest">View All Collections</button>' +
        '</div>';
    ...
    grid.className = 'grid grid-cols-2 gap-3 sm:gap-6 lg:gap-8';
  ...
  function renderCartDrawer() {
    ...
    var itemsHTML = items.length === 0
      ? '<div class="h-full flex flex-col items-center justify-center text-center space-y-4 py-12">' +
        '<div class="w-16 h-16 bg-[#0d2a18] rounded-full flex items-center justify-center text-amber-300 border border-amber-400/30">' + icon('shoppingBag', 'w-8 h-8') + '</div>' +
        '<div><h3 class="font-serif-heading text-lg font-bold text-neutral-900">Your Cart is empty</h3>' +
        '<p class="text-xs text-neutral-500 mt-1 max-w-xs">Explore our luxury dining, home decor, and fashion goods.</p></div>' +
        '<button data-close-cart class="bg-[#0a0a0a] text-amber-300 text-xs font-bold px-6 py-3 rounded-lg uppercase tracking-widest hover:bg-black transition-colors border border-amber-400/30">START SHOPPING</button>' +
        '</div>'
    ...
    var footerHTML = items.length === 0 ? '' :
      ...
      '<input type="text" id="promo-input" value="' + esc(cartUiState.promoCode) + '" placeholder="PROMO (e.g. PENTAGON10)" class="w-full bg-neutral-50 border border-neutral-300 text-xs rounded-lg pl-8 pr-3 py-2 uppercase font-mono focus:outline-none focus:border-emerald-800" /></div>' +
      '<button type="submit" class="bg-[#0a0a0a] text-amber-300 text-xs font-bold px-4 py-2 rounded-lg hover:bg-black uppercase cursor-pointer border border-amber-400/30">Apply</button></form>' +
      (cartUiState.promoError ? '<p class="text-[11px] text-rose-600">' + esc(cartUiState.promoError) + '</p>' : '') +
      (cartUiState.promoSuccess ? '<p class="text-[11px] text-emerald-800 font-semibold flex items-center gap-1">' + icon('check', 'w-3.5 h-3.5 text-emerald-600') + esc(cartUiState.promoSuccess) + '</p>' : '') +
      ...
      (cartUiState.discountPercent > 0 ? '<div class="flex justify-between text-emerald-800 font-medium"><span>Discount (' + cartUiState.discountPercent + '%)</span><span>-' + esc(formatPrice(discountAmount, state.currency)) + '</span></div>' : '') +
      '<div class="flex justify-between"><span>Estimated Delivery</span><span class="font-semibold text-neutral-900">' + (amountNeeded === 0 ? 'COMPLIMENTARY' : esc(formatPrice(15, state.currency))) + '</span></div>' +
      ...
      '</div>' +
      '<button id="proceed-checkout" class="w-full bg-[#0a0a0a] hover:bg-[#0d2a18] text-white py-3.5 text-xs font-bold tracking-widest uppercase rounded-lg transition-colors flex items-center justify-center space-x-2 cursor-pointer shadow-md border border-amber-400/40">' +
      '<span>PROCEED TO CHECKOUT</span>' + icon('arrowRight', 'w-4 h-4 text-amber-300') + '</button>' +
      '</div>';

    root.innerHTML =
      '<div class="fixed inset-0 z-50 overflow-hidden">' +
      '<div class="fixed inset-0 bg-black/80 backdrop-blur-xs transition-opacity" data-close-cart></div>' +
      '<div class="fixed inset-y-0 right-0 max-w-full flex pl-10"><div class="w-screen max-w-md bg-[#fdfbf7] shadow-2xl flex flex-col justify-between border-l border-amber-500/30">' +
      '<div class="p-6 border-b border-amber-500/20 bg-[#0d2a18] text-white">' +
      '<div class="flex items-center justify-between"><div class="flex items-center space-x-2">' + icon('shoppingBag', 'w-5 h-5 text-amber-300') +
      '<h2 class="font-serif-heading text-lg font-bold tracking-wider text-white uppercase">YOUR SHOPPING CART</h2>' +
      '<span class="text-xs bg-amber-400 text-black font-bold px-2 py-0.5 rounded-full">' + items.reduce(function (a, i) { return a + i.quantity; }, 0) + '</span></div>' +
      '<button data-close-cart class="p-2 text-amber-200 hover:text-white transition-colors cursor-pointer" aria-label="Close cart">' + icon('x', 'w-5 h-5') + '</button></div>' +
      '<div class="mt-4 pt-4 border-t border-amber-500/20"><div class="flex justify-between text-xs font-semibold uppercase tracking-wider mb-1.5">' +
      (amountNeeded === 0
        ? '<span class="text-amber-300 flex items-center gap-1 font-bold">' + icon('sparkles', 'w-3.5 h-3.5') + 'COMPLIMENTARY EXPRESS DELIVERY UNLOCKED</span>'
        : '<span class="text-amber-100/90">Add ' + esc(formatPrice(amountNeeded, state.currency)) + ' for Free Express Shipping</span>') +
      '<span class="text-amber-300 font-mono">' + progressPercent + '%</span></div>' +
      '<div class="w-full h-2 bg-[#0a0a0a] rounded-full overflow-hidden border border-amber-400/20"><div class="h-full transition-all duration-500 ' + (amountNeeded === 0 ? 'bg-amber-400' : 'bg-emerald-500') + '" style="width:' + progressPercent + '%"></div></div>' +
      '</div></div>' +
  ...
  function renderWishlistDrawer() {
    ...
    var itemsHTML = state.wishlist.length === 0
      ? '<div class="h-full flex flex-col items-center justify-center text-center space-y-4 py-12">' +
        '<div class="w-16 h-16 bg-[#0d2a18] text-amber-300 rounded-full flex items-center justify-center border border-amber-400/30">' + icon('heart', 'w-8 h-8') + '</div>' +
        ...
        '<button data-close-wishlist class="bg-[#0a0a0a] text-amber-300 text-xs font-bold px-6 py-3 rounded-lg uppercase tracking-widest hover:bg-black border border-amber-400/30 cursor-pointer">EXPLORE ARCHIVE</button>' +
        '</div>'
      : state.wishlist.map(function (pid) {
          ...
          return '<div class="py-4 flex space-x-4">' +
            ...
            '<button data-quickview-trigger data-product-id="' + esc(pid) + '" data-close-wishlist-after class="mt-3 bg-[#0a0a0a] hover:bg-[#0d2a18] text-amber-300 text-[11px] font-bold py-2 px-3 rounded-lg uppercase tracking-wider flex items-center justify-center space-x-1.5 cursor-pointer border border-amber-400/30">' +
            icon('shoppingBag', 'w-3.5 h-3.5 text-amber-300') + '<span>View &amp; Add</span></button>' +
            '</div></div>';
        }).join('');
    ...
    root.innerHTML =
      ...
      '<div class="fixed inset-y-0 right-0 max-w-full flex pl-10"><div class="w-screen max-w-md bg-[#fdfbf7] shadow-2xl flex flex-col justify-between border-l border-amber-500/30">' +
      '<div class="p-6 border-b border-amber-500/20 bg-[#0d2a18] text-white flex items-center justify-between">' +
      ...
      '<span class="text-xs bg-amber-400 text-black font-bold px-2 py-0.5 rounded-full">' + state.wishlist.length + '</span></div>' +
      '<button data-close-wishlist class="p-2 text-amber-200 hover:text-white cursor-pointer">' + icon('x', 'w-5 h-5') + '</button></div>' +
      '<div class="flex-1 overflow-y-auto p-6 divide-y divide-neutral-200/80">' + itemsHTML + '</div>' +
      '</div></div></div>';
  ...
  function openQuickView(productId) {
    ...
    if (!product) return;
    qvState = { productId: productId, selectedImg: 0, selectedColor: product.colors[0], selectedSize: product.sizes[0], quantity: 1, addedToast: false };
  ...
  function renderQuickView() {
    ...
    var thumbsHTML = product.images.length > 1
      ? '<div class="flex gap-2 overflow-x-auto pb-1">' + product.images.map(function (img, idx) {
          return '<button data-qv-thumb="' + idx + '" class="relative w-16 aspect-square rounded-lg overflow-hidden border-2 transition-all cursor-pointer ' + (qvState.selectedImg === idx ? 'border-amber-500 scale-105' : 'border-transparent opacity-70 hover:opacity-100') + '"><img src="' + esc(imgUrl(img)) + '" alt="" class="w-full h-full object-cover" /></button>';
        }).join('') + '</div>'
      : '';

    var colorsHTML = product.colors.map(function (c) {
      var active = qvState.selectedColor.name === c.name;
      return '<button data-qv-color="' + esc(c.name) + '" class="flex items-center space-x-2 px-3 py-1.5 rounded-md border text-xs cursor-pointer ' + (active ? 'border-black bg-[#0a0a0a] text-amber-300 font-bold' : 'border-neutral-300 bg-white text-neutral-800 hover:border-neutral-400') + '">' +
        '<span class="w-3 h-3 rounded-full border border-white/20" style="background-color:' + esc(c.hex) + '"></span><span>' + esc(c.name) + '</span></button>';
    }).join('');
    ...
    root.innerHTML =
      ...
      '<div class="relative w-full max-w-4xl bg-[#fdfbf7] rounded-xl shadow-2xl z-10 overflow-hidden border border-amber-500/30 my-6">' +
      '<button data-close-qv class="absolute top-4 right-4 z-20 p-2 bg-[#0a0a0a]/80 hover:bg-[#0a0a0a] text-amber-300 rounded-full transition-colors shadow-md cursor-pointer border border-amber-400/30" aria-label="Close modal">' + icon('x', 'w-5 h-5') + '</button>' +
      '<div class="grid grid-cols-1 md:grid-cols-2">' +
      '<div class="p-6 bg-[#f5f2e9] flex flex-col justify-between">' +
      ...
      '<span class="text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded-md font-bold text-[10px]">' + (product.inStock ? 'IN STOCK • EXPRESS DISPATCH' : 'PRE-ORDER') + '</span>' +
      '</div>' +
      '<h2 class="font-serif-heading text-xl sm:text-2xl font-bold text-[#0a0a0a]">' + esc(product.name) + '</h2>' +
      '<div class="flex items-center space-x-3 mt-2"><div class="flex items-center text-amber-500">' + icon('star', 'w-4 h-4 fill-amber-400 text-amber-400', 'currentColor') +
      ...
      '<button data-qv-add class="flex-1 bg-[#0a0a0a] hover:bg-[#0d2a18] text-white py-3 px-6 text-xs font-bold tracking-widest uppercase rounded-lg transition-colors flex items-center justify-center space-x-2 cursor-pointer shadow-md border border-amber-400/30">' +
      icon('shoppingBag', 'w-4 h-4 text-amber-300') + '<span>ADD TO CART</span></button>' +
      '</div>' +
      (qvState.addedToast ? '<div class="bg-[#0d2a18] text-amber-300 text-xs font-bold py-2 px-3 rounded-lg flex items-center justify-center space-x-2 border border-amber-400/30">' + icon('check', 'w-4 h-4 text-amber-300') + '<span>Item added to your Cart!</span></div>' : '') +
  ...
  function renderSearchModal() {
    ...
    var results = q ? PRODUCTS.filter(function (p) {
      return p.name.toLowerCase().indexOf(q) !== -1 ||
        ...
        p.fabric.toLowerCase().indexOf(q) !== -1;
    }) : [];

    var bodyHTML = !searchQuery
      ? '<div class="space-y-3"><p class="text-xs font-bold text-emerald-900 uppercase tracking-widest flex items-center gap-1">' + icon('sparkles', 'w-3.5 h-3.5 text-amber-500') + 'Popular Search Terms</p>' +
        '<div class="flex flex-wrap gap-2">' + popularKeywords.map(function (kw) {
          return '<button data-search-kw="' + esc(kw) + '" class="bg-white hover:bg-[#0a0a0a] hover:text-amber-300 text-neutral-800 text-xs font-semibold px-3.5 py-1.5 rounded-full border border-neutral-300 transition-colors cursor-pointer">' + esc(kw) + '</button>';
        }).join('') + '</div></div>'
      : '<div class="max-h-[60vh] overflow-y-auto space-y-3 pr-1">' +
        ...
        (results.length === 0
          ...
          : '<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">' + results.map(function (p) {
              return '<div data-search-result="' + esc(p.id) + '" class="flex items-center space-x-3 p-3 bg-white border border-neutral-200 rounded-lg hover:border-emerald-800 transition-colors cursor-pointer group shadow-xs">' +
                '<img src="' + esc(imgUrl(p.images[0])) + '" alt="' + esc(p.name) + '" class="w-16 h-20 object-cover rounded-md" />' +
                '<div class="flex-1 min-w-0"><p class="text-[10px] text-emerald-800 font-bold uppercase tracking-wider">' + esc(p.subCategory) + '</p>' +
                '<h4 class="font-serif-heading text-sm font-bold text-neutral-900 truncate group-hover:text-emerald-900">' + esc(p.name) + '</h4>' +
                '<p class="text-xs font-bold text-[#8b1c1c] mt-1">' + esc(formatPrice(p.price, state.currency)) + '</p></div>' +
                icon('arrowRight', 'w-4 h-4 text-neutral-400 group-hover:text-emerald-800 transition-colors') + '</div>';
            }).join('') + '</div>') +
        '</div>';
    ...
    root.innerHTML =
      ...
      '<div class="relative w-full max-w-3xl bg-[#fdfbf7] rounded-xl shadow-2xl z-10 overflow-hidden border border-amber-500/30 p-6 space-y-6">' +
      '<div class="flex items-center justify-between border-b border-neutral-300 pb-4">' +
      '<div class="flex items-center space-x-3 flex-1 pr-4">' + icon('search', 'w-5 h-5 text-emerald-800') +
  ...
  function renderCheckoutModal() {
    ...
    if (checkoutState.step === 'processing') {
      ...
      var paymentFieldsHTML = f.paymentMethod === 'mpesa'
        ? '<div class="p-4 bg-emerald-50 border border-emerald-300 rounded-lg space-y-2">' +
          '<label class="text-[11px] font-bold text-emerald-900 uppercase block">M-Pesa Phone Number (Prompt will be sent to this phone)</label>' +
          '<input type="text" required id="f-mpesaPhone" value="' + esc(f.mpesaPhone) + '" placeholder="254712345678" class="w-full bg-white border border-emerald-400 rounded-lg p-2 text-xs font-mono font-bold focus:outline-none focus:border-emerald-700" />' +
          '<p class="text-[11px] text-emerald-800">You will receive an M-Pesa STK push prompt on your phone upon clicking Place Order.</p></div>'
      ...
      bodyHTML =
        ...
        '<button type="button" data-payment-method="mpesa" class="py-3 px-3 rounded-lg text-xs font-bold flex items-center justify-center space-x-2 border cursor-pointer ' + (f.paymentMethod === 'mpesa' ? 'bg-[#0d2a18] text-amber-300 border-amber-400 ring-2 ring-amber-400/50' : 'bg-white text-neutral-900 border-neutral-300') + '">' + icon('smartphone', 'w-4 h-4 text-emerald-500') + '<span>M-Pesa Express</span></button>' +
        '<button type="button" data-payment-method="card" class="py-3 px-3 rounded-lg text-xs font-bold flex items-center justify-center space-x-2 border cursor-pointer ' + (f.paymentMethod === 'card' ? 'bg-[#0a0a0a] text-amber-300 border-amber-400 ring-2 ring-amber-400/50' : 'bg-white text-neutral-900 border-neutral-300') + '">' + icon('creditCard', 'w-4 h-4 text-amber-500') + '<span>Credit / Debit Card</span></button>' +
        '</div></div>' +
        ...
        '<div class="grid grid-cols-2 gap-3">' +
        '<div><label class="text-[11px] font-bold text-neutral-600 uppercase">First Name</label><input type="text" required id="f-firstName" value="' + esc(f.firstName) + '" class="w-full bg-white border border-neutral-300 rounded-lg p-2 text-xs focus:outline-none focus:border-emerald-800" /></div>' +
        '<div><label class="text-[11px] font-bold text-neutral-600 uppercase">Last Name</label><input type="text" required id="f-lastName" value="' + esc(f.lastName) + '" class="w-full bg-white border border-neutral-300 rounded-lg p-2 text-xs focus:outline-none focus:border-emerald-800" /></div>' +
        '</div>' +
        '<div class="grid grid-cols-2 gap-3">' +
        '<div><label class="text-[11px] font-bold text-neutral-600 uppercase">Email Address</label><input type="email" required id="f-email" value="' + esc(f.email) + '" class="w-full bg-white border border-neutral-300 rounded-lg p-2 text-xs focus:outline-none focus:border-emerald-800" /></div>' +
        '<div><label class="text-[11px] font-bold text-neutral-600 uppercase">Phone Number</label><input type="tel" required id="f-phone" value="' + esc(f.phone) + '" class="w-full bg-white border border-neutral-300 rounded-lg p-2 text-xs focus:outline-none focus:border-emerald-800" /></div>' +
        '</div>' +
        '<div><label class="text-[11px] font-bold text-neutral-600 uppercase">Street Address / Estate</label><input type="text" required id="f-address" value="' + esc(f.address) + '" class="w-full bg-white border border-neutral-300 rounded-lg p-2 text-xs focus:outline-none focus:border-emerald-800" /></div>' +
        '<div class="grid grid-cols-3 gap-3">' +
        '<div><label class="text-[11px] font-bold text-neutral-600 uppercase">City</label><input type="text" required id="f-city" value="' + esc(f.city) + '" class="w-full bg-white border border-neutral-300 rounded-lg p-2 text-xs focus:outline-none focus:border-emerald-800" /></div>' +
        '<div><label class="text-[11px] font-bold text-neutral-600 uppercase">Postcode</label><input type="text" required id="f-postalCode" value="' + esc(f.postalCode) + '" class="w-full bg-white border border-neutral-300 rounded-lg p-2 text-xs focus:outline-none focus:border-emerald-800" /></div>' +
        '<div><label class="text-[11px] font-bold text-neutral-600 uppercase">Country</label><input type="text" required id="f-country" value="' + esc(f.country) + '" class="w-full bg-white border border-neutral-300 rounded-lg p-2 text-xs focus:outline-none focus:border-emerald-800" /></div>' +
        '</div></div>' +
        paymentFieldsHTML +
        '<button type="submit" class="w-full bg-[#0a0a0a] hover:bg-[#0d2a18] text-amber-300 py-4 text-xs font-bold tracking-widest uppercase rounded-lg transition-colors flex items-center justify-center space-x-2 cursor-pointer shadow-lg border border-amber-400/40">' +
        icon('lock', 'w-4 h-4 text-amber-300') + '<span>CONFIRM &amp; PLACE ORDER — ' + esc(formatPrice(totals.total, state.currency)) + '</span></button>' +
        '</div>' +
        '<div class="lg:col-span-5 p-5 sm:p-8 bg-[#f5f2e9] border-l border-neutral-200 flex flex-col justify-between">' +
        ...
        (totals.discountAmount > 0 ? '<div class="flex justify-between text-emerald-800 font-semibold"><span>Applied Discount</span><span>-' + esc(formatPrice(totals.discountAmount, state.currency)) + '</span></div>' : '') +
        ...
        '</div></div>' +
        '<div class="mt-6 p-3 bg-white border border-amber-500/30 rounded-lg text-[11px] text-neutral-600 space-y-1">' +
        '<p class="flex items-center gap-1 font-bold text-emerald-900">' + icon('shieldCheck', 'w-4 h-4 text-emerald-700') + 'Pentagon Authenticity Guarantee</p>' +
      ...
      bodyHTML =
        ...
        '<div class="w-16 h-16 bg-emerald-100 text-emerald-800 rounded-full flex items-center justify-center mx-auto border-2 border-emerald-500">' + icon('checkCircle', 'w-10 h-10 text-emerald-700') + '</div>' +
        '<div class="space-y-2"><span class="text-xs font-bold uppercase tracking-widest text-amber-700">Order Received &amp; Verified</span>' +
        ...
        '<div class="bg-white p-6 border border-amber-500/30 rounded-xl text-left space-y-4 text-xs shadow-xs">' +
        ...
        '</div>' +
        '<div class="bg-[#f5f2e9] border border-amber-500/20 rounded-xl p-4 text-xs text-neutral-600 text-left">' +
        '<p class="font-bold text-neutral-800 uppercase tracking-wider text-[10px] mb-1">Track this order anytime</p>' +
        '<p>Sign in at <a href="' + siteUrl('account/login') + '" class="text-emerald-800 font-semibold hover:underline">My Account</a> using ' + (o.customer.email ? 'the email <strong>' + esc(o.customer.email) + '</strong>' : 'the phone number <strong>' + esc(o.customer.phone) + '</strong>') + ' you just used — no password needed.</p>' +
        '</div>' +
        '<div class="flex flex-col sm:flex-row gap-3 justify-center">' +
        '<button data-close-checkout class="bg-[#0a0a0a] hover:bg-[#0d2a18] text-amber-300 py-3.5 px-8 text-xs font-bold tracking-widest uppercase rounded-lg transition-colors border border-amber-400/30 cursor-pointer">CONTINUE SHOPPING</button>' +
        '<a href="' + siteUrl('account/login') + '" class="inline-flex items-center justify-center bg-white hover:bg-neutral-50 text-[#0a0a0a] py-3.5 px-8 text-xs font-bold tracking-widest uppercase rounded-lg transition-colors border-2 border-[#0a0a0a]">TRACK MY ORDERS</a>' +
        '</div>' +
        '</div>';
    }
    ...
    root.innerHTML =
      ...
      '<div class="relative w-full max-w-4xl bg-[#fdfbf7] rounded-xl shadow-2xl z-10 overflow-hidden border border-amber-500/30 my-6">' +
      '<div class="p-4 sm:p-5 bg-[#0d2a18] text-white border-b border-amber-500/20 flex items-center justify-between">' +
      '<div class="flex items-center gap-2"><div class="w-7 h-7 bg-[#0a0a0a] text-amber-300 flex items-center justify-center rounded-md border border-amber-400/30"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="w-4 h-4"><polygon points="12,2 22,9 18,21 6,21 2,9" /></svg></div>' +
      '<span class="font-serif-heading text-lg font-bold tracking-widest text-white uppercase">PENTAGON CHECKOUT</span></div>' +
      '<button data-close-checkout class="p-2 text-amber-200 hover:text-white cursor-pointer">' + icon('x', 'w-5 h-5') + '</button></div>' +
      bodyHTML +
      '</div></div>';
  ...
  function logoLoaderHTML(message) {
    return '<div class="flex flex-col items-center justify-center p-6 text-center select-none">' +
      ...
      '<div class="absolute w-16 h-16 sm:w-20 sm:h-20 bg-emerald-700/20 rounded-full blur-xl animate-pulse"></div>' +
      '<div class="relative w-12 h-12 sm:w-16 sm:h-16 bg-[#0a0a0a] text-amber-300 flex items-center justify-center rounded-xl border border-amber-400/50 shadow-2xl animate-logo-fade">' +
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="w-6 h-6 sm:w-8 sm:h-8 text-amber-300 drop-shadow-sm"><polygon points="12,2 22,9 18,21 6,21 2,9" /></svg></div></div>' +
      ...
      '<span class="block text-[9px] sm:text-[10px] tracking-[0.35em] text-amber-700 font-sans font-bold uppercase">COLLECTIONS</span></div>' +
  ...
  function lookbookMainHTML(look) {
    var hotspots = look.products.map(function (item) {
      return '<div class="absolute transform -translate-x-1/2 -translate-y-1/2 group z-20 cursor-pointer" style="left:' + item.xPercent + '%;top:' + item.yPercent + '%" data-quickview-trigger data-product-id="' + esc(item.product.id) + '">' +
        ...
        '<p class="text-[10px] text-amber-800 font-bold mt-0.5">$' + item.product.price + '</p>' +
    ...
    return '<img src="' + esc(imgUrl(look.mainImage)) + '" alt="' + esc(look.title) + '" class="w-full h-full object-cover object-center filter brightness-95" />' +
      ...
      hotspots +
      '<div class="absolute bottom-6 left-6 max-w-lg text-left"><span class="text-xs font-mono text-amber-300 tracking-widest uppercase block mb-1">' + esc(look.subtitle) + '</span>' +
  ...
  function lookbookListHTML(look) {
    return look.products.map(function (item) {
      var p = item.product;
      return '<div data-quickview-trigger data-product-id="' + esc(p.id) + '" class="flex items-center space-x-3 p-3 bg-neutral-900/90 border border-neutral-800 rounded-xs hover:border-amber-300 transition-colors cursor-pointer group">' +
        '<img src="' + esc(imgUrl(p.images[0])) + '" alt="' + esc(p.name) + '" class="w-16 h-20 object-cover rounded-xs" />' +
        '<div class="flex-1 min-w-0"><p class="text-[10px] text-amber-300 font-mono uppercase tracking-widest">' + esc(p.subCategory) + '</p>' +
        '<h5 class="font-serif-heading text-sm font-bold text-white group-hover:text-amber-200 truncate">' + esc(p.name) + '</h5>' +
        '<p class="text-xs font-bold text-neutral-300 mt-1">$' + p.price + '</p></div>' +
        '<div class="p-2 bg-neutral-800 group-hover:bg-amber-200 group-hover:text-black rounded-xs transition-colors">' + icon('eye', 'w-4 h-4') + '</div></div>';
    }).join('');
  }
  ...
  function initGlobalDelegation() {
    document.addEventListener('click', function (e) {
      ...
      if (buyNow) {
        ...
        addToCart(pid1, p1.colors[0], p1.sizes[0], 1);
        openCheckoutModal();
        return;
      }
      ...
      if (quickAdd) {
        ...
        addToCart(pid2, p2.colors[0], p2.sizes[0], 1);
        openCartDrawer();
        return;
      }
