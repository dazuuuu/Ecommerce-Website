...
<div class="flex items-center justify-between mb-5">
  ...
  <a href="<?= url('/admin/categories/create') ?>" class="bg-[#0a0a0a] text-amber-300 text-xs font-bold px-4 py-2.5 rounded-lg uppercase tracking-widest hover:bg-black transition-colors border border-amber-400/30">+ Add Category</a>
</div>
  ...
  <?php foreach ($categories as $cat): ?>
    <div class="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
      <div class="h-32 bg-neutral-100 relative">
        ...
        <?php endif; ?>
        <span class="absolute top-2 left-2 bg-black/70 text-amber-300 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded"><?= e($cat['category_key']) ?></span>
      </div>
      <div class="p-4">
        ...
        <div class="flex items-center gap-3 mt-3 pt-3 border-t border-neutral-100">
          <a href="<?= url('/admin/categories/' . (int) $cat['id'] . '/edit') ?>" class="text-xs font-bold text-emerald-800 hover:underline">Edit</a>
          <?php if ($cat['image']): ?>
