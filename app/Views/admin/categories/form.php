...
<form method="post" action="<?= $actionUrl ?>" enctype="multipart/form-data" class="max-w-2xl bg-white border border-neutral-200 rounded-xl shadow-sm p-6 space-y-5">
  ...
  <div class="grid grid-cols-2 gap-4">
    <div>
      ...
      <input type="text" name="category_key" value="<?= e($form['category_key']) ?>" <?= $category ? 'readonly' : 'placeholder="auto-generated from name if left blank"' ?> class="w-full mt-1 border border-neutral-300 rounded-lg p-2.5 text-sm <?= $category ? 'bg-neutral-100 text-neutral-500' : 'bg-white' ?> focus:outline-none focus:border-emerald-800" />
    </div>
    <div>
      <label class="text-[11px] font-bold text-neutral-600 uppercase">Sort Order</label>
      <input type="number" name="sort_order" value="<?= (int) $form['sort_order'] ?>" class="w-full mt-1 bg-white border border-neutral-300 rounded-lg p-2.5 text-sm focus:outline-none focus:border-emerald-800" />
    </div>
  </div>
  ...
  <div>
    ...
    <input type="text" name="tagline" value="<?= e($form['tagline']) ?>" class="w-full mt-1 bg-white border border-neutral-300 rounded-lg p-2.5 text-sm focus:outline-none focus:border-emerald-800" />
  </div>
