 ...
 *   $activeNav  — one of: dashboard, products, product-form, categories, offers, orders, seo
...
<body class="bg-[#faf9f6] text-[#1a1a1a] antialiased min-h-screen flex">

  <!-- Sidebar -->
  <aside class="w-60 shrink-0 bg-[#0c2e1b] text-white min-h-screen flex flex-col justify-between border-r border-[#1b432a] sticky top-0 self-start h-screen overflow-y-auto">
    <div>
      <div class="flex items-center gap-2.5 px-5 py-5 border-b border-[#1b432a]">
        <div class="w-8 h-8 bg-[#0a0a0a] text-amber-300 flex items-center justify-center rounded-lg border border-amber-400/40 shrink-0">
          <?= pentagonLogoSvg('w-4 h-4 text-amber-300') ?>
        </div>
        <div class="flex flex-col leading-none">
          <span class="font-serif-heading text-sm font-extrabold tracking-[0.15em] uppercase">PENTAGON</span>
      ...
      <nav class="p-3 space-y-1 text-xs font-semibold uppercase tracking-wider">
        <?php foreach ($navItems as $item): $active = ($activeNav ?? '') === $item['id']; ?>
          <a href="<?= e($item['href']) ?>" class="block px-3 py-2.5 rounded-lg transition-colors <?= $active ? 'bg-amber-400 text-black font-bold' : 'text-amber-100/80 hover:bg-[#184229] hover:text-white' ?>">
            <?= e($item['label']) ?>
          </a>
        <?php endforeach; ?>
      </nav>
    ...
    <div class="p-4 border-t border-[#1b432a] text-xs">
      <p class="text-amber-200/60">Signed in as</p>
      <p class="font-bold text-amber-300 mb-2 truncate"><?= e($admin['email'] ?? '') ?></p>
      <div class="flex flex-col gap-1.5">
        <a href="<?= url('/') ?>" target="_blank" class="text-amber-100/70 hover:text-white">View storefront &rarr;</a>
  ...
  <div class="flex-1 min-w-0">
    <header class="bg-white border-b border-neutral-200 px-6 py-4 sticky top-0 z-10">
