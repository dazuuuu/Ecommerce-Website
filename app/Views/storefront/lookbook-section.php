...
<div id="lookbook-section" class="py-20 bg-[#141414] text-white">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="rounded-xl border border-neutral-800 bg-neutral-900 p-10 text-center">
      <p class="text-sm uppercase tracking-widest text-amber-300">Lookbook unavailable</p>
...
<section class="py-20 bg-[#141414] text-white">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 pb-4 border-b border-neutral-800">
      <div>
        <span class="text-xs font-bold text-amber-300 uppercase tracking-widest flex items-center gap-1.5 mb-1">
      ...
      <div class="flex items-center space-x-3 mt-4 md:mt-0" id="lookbook-tabs">
        <?php foreach ($lookbook as $idx => $look): ?>
          <button data-lookbook-index="<?= $idx ?>" class="lookbook-tab px-4 py-2 text-xs font-mono tracking-widest uppercase rounded-xs transition-colors cursor-pointer <?= $idx === 0 ? 'bg-amber-200 text-[#1a1a1a] font-bold' : 'bg-neutral-900 text-neutral-400 hover:text-white border border-neutral-800' ?>"><?= e($look['season']) ?></button>
        <?php endforeach; ?>
      </div>
    </div>
